...
class Blockchain(object):
    ...
    # 创建新交易
    def new_transaction(self, sender, recipient, amount,h):
    ...
    def new_pk(self, sender, recipient, pk):
        # Adds a new pk to the list of transactions(向交易列表中添加一个新的公钥)
        """
                生成新交易信息，此交易信息将加入到下一个待挖的区块中
                :param sender: Address of the Sender  # 发送方
                :param recipient: Address of the Recipient # 接收方
    ...
    def new_ct(self, sender, recipient,ct,hc_t,hct,S):
        # Adds a new coordinate to the list of transactions(向交易列表中添加一个新的坐标)
        """
                生成新交易信息，此交易信息将加入到下一个待挖的区块中
                :param sender: Address of the Sender  # 发送方
                :param recipient: Address of the Recipient # 接收方
                :param ct: ct  # 含s密文坐标
                :param c_t:c_t # 无s密文坐标
                :param hc_t:hc_t#无s密文坐标的hash值
                :param hct:hct#含s密文坐标的hash值
                :return: The index of the Block that will hold this transaction # 需要将交易记录在下一个区块中
        """
        self.currentTransaction.append({
            'submit_time':time(),
    ...
    def new_k(self, sender, recipient, k,D,R,task):
        # Adds a new k to the list of transactions(向交易列表给所有参与方发位置坐标k)
        """
                生成新交易信息，此交易信息将加入到下一个待挖的区块中
                :param sender: Address of the Sender  # 发送方
                :param recipient: Address of the Recipient # 接收方
    ...
    def new_sl(self, sender, recipient, hc_t,d,pi,ct):
        # Adds a new pk to the list of transactions(向交易列表中添加一个新的公钥)
        """
                生成新交易信息，此交易信息将加入到下一个待挖的区块中
                :param sender: Address of the Sender  # 发送方
...
def new_transaction():
    ...
    # 检查所需字段是否在过账数据中
    required = ['sender', 'recipient', 'amount','h']
    if not all(k in values for k in required):
        return 'Missing values', 400  # HTTP状态码等于400表示请求错误
    if hash(values['amount'])==values['h']:   
    # 创建新交易
        index = blockchain.new_transaction(values['sender'], values['recipient'], values['amount'],values['h'])
        response = {'message': f'Transaction will be added to Block {index}'}
        return jsonify(response), 201
    #hash不对就报错
    else:
        print(hash(values['amount']),values['h'],'hash 不一致')
        return 'hash error', 400



    
@app.route('/pk/new', methods=['POST'])###写入pk
def new_pk():
    # 将请求参数做了处理，得到的是字典格式的，因此排序会打乱依据字典排序规则
    values = request.get_json()
 
    # 检查所需字段是否在过账数据中
    required = ['sender', 'recipient', 'pk']
    if not all(k in values for k in required):
        return 'Missing values', 400  # HTTP状态码等于400表示请求错误
    if isinstance(values['pk'],list) and len(values['pk'])==2 and len(values['pk'][0])==2 and len(values['pk'][1])==2 :   
    # 格式对创建新pk
        index = blockchain.new_pk(values['sender'], values['recipient'], values['pk'])
        response = {'message': f'Transaction will be added to Block {index}'}
        return jsonify(response), 201
    #格式不对就报错
    else:
        print(type(values['pk']),len(values['pk']),len(values['pk'][0]),len(values['pk'][1]),'pk格式错误')
        return 'pk error', 400


@app.route('/ct/new', methods=['POST'])###写入ct
def new_ct():
    # 将请求参数做了处理，得到的是字典格式的，因此排序会打乱依据字典排序规则
    values = request.get_json()
 
    # 检查所需字段是否在过账数据中
    required = ['sender', 'recipient','ct','hc_t','hct','$S']
    if not all(k in values for k in required):
        return 'Missing values', 400  # HTTP状态码等于400表示请求错误
    if hash(values['ct'])==values['hct'] and len(values['hct'])==len(values['hc_t']):# 含s密文坐标hash对应且两个都是hash格式则添加ct
        if values['$S']>=10:#保证金需大于10
            index = blockchain.new_ct(values['sender'], values['recipient'], values['ct'],values['hc_t'],values['hct'],values['$S'])
            response = {'message': f'Transaction will be added to Block {index}'}
            return jsonify(response), 201
        else:
            print(values['$S'],'保证金不够！')
            return 'not enough guaranty money!', 400
    #格式不对就报错
    else:
        print(hash(values['hct']),'hash值不对应！')
        return 'hash error', 400


@app.route('/k/new', methods=['POST'])###写入k
def new_k():
    # 将请求参数做了处理，得到的是字典格式的，因此排序会打乱依据字典排序规则
    values = request.get_json()
 
    # 检查所需字段是否在过账数据中
    required = ['sender', 'recipient', 'k','D','$R','task']
    if not all(k in values for k in required):
        return 'Missing values', 400  # HTTP状态码等于400表示请求错误
    if isinstance(values['k'],list) and len(values['k'])==4 and len(values['k'][0])==2 and len(values['k'][1])==2 :   
    # 格式对创建新pk
        index = blockchain.new_k(values['sender'],values['recipient'],values['k'],values['D'],values['$R'],values['task'])
        response = {'message': f'Transaction will be added to Block {index}'}
        return jsonify(response), 201
    #格式不对就报错
    else:
        print(type(values['k']),len(values['k']),len(values['k'][0]),len(values['k'][1]),values['k'][2],values['k'][3],'k格式错误')
        return 'k error', 400


@app.route('/sl/new', methods=['POST'])###写入pk
def new_sl():
    # 将请求参数做了处理，得到的是字典格式的，因此排序会打乱依据字典排序规则
    values = request.get_json()
 
    # 检查所需字段是否在过账数据中
    required = ['sender', 'recipient', 'hc_t', 'd', 'pi', 'ct']
    if not all(k in values for k in required):
        return 'Missing values', 400  # HTTP状态码等于400表示请求错误
    if len(values['hc_t'])==64:  #和之前的hash值一致 
    # 格式对创建新sl
        index = blockchain.new_sl(values['sender'], values['recipient'], values['hc_t'],values['d'],values[ 'pi'], values['ct'])
        response = {'message': f'Transaction will be added to Block {index}'}
        return jsonify(response), 201
    #格式不对就报错
    else:
        print(type(values['hc_t']),len(values['hc_t']),'hc_t格式错误')
        return 'hc_t error', 400
